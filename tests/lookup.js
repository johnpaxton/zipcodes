...
let zipcodes = require(path.join(__dirname, '../', 'lib'));
...
let tests = {
  'should export': {
    ...
    'a functions and objects': function (d) {
      ['lookup', 'lookupByName', 'distance', 'radius', 'toMiles', 'toKilometers'].forEach(function (
        key
      ) {
  ...
  distance: {
    ...
    'should find': function () {
      let dist = zipcodes.distance(62959, 90210);
      assert.equal(dist, 1662);

      let dist2 = zipcodes.distance(62959, 62959);
      assert.equal(dist2, 0);

      let dist3 = zipcodes.distance(62959, 63801);
      assert.equal(dist3, 68);

      let dist4 = zipcodes.distance(62959, 95014);
      ...
      let dist5 = zipcodes.distance(62959, 90210);
      assert.equal(dist5, 1662);

      let dist6 = zipcodes.distance('T2E', 'V5N');
      assert.equal(dist6, 417);
    },
    'should not find': function () {
      let dist = zipcodes.distance(62959, 123456);
      assert.equal(dist, null);

      let dist2 = zipcodes.distance(123456, 62959);
      assert.equal(dist2, null);

      let dist3 = zipcodes.distance(123456, 'V5N');
  ...
  lookups: {
    ...
    'should find by coordinates': function () {
      let loc = zipcodes.lookupByCoords(29.2108, -81.0228);
  ...
  radius: {
    ...
    'should find': function () {
      let rad = zipcodes.radius(null, 20);
      assert.equal(rad.length, 0);

      rad = zipcodes.radius(62959, 20);
      assert.equal(rad.length, 38);

      rad = zipcodes.radius(95014, 50);
      assert.equal(rad.length, 387);

      rad = zipcodes.radius(95014, 50, true);
      assert.equal(rad.length, 387);
